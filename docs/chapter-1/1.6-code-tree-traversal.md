...
to *preorder* traversal, and differs in code only in that we use a new type
of object called a `deque` that is imported from the Python standard library
module `collections`. This is a double-ended queue, which is similar to a 
list but can append or pop from either end, instead of just the right side.
So we use the `deque` as a queue this time, and `pop` Nodes from the left
...
topological sort, where children come before parents.
...
information about the tree or measuring statistics on a tree. 
...
just add 1. A simple function is written below that uses *preorder* traversal
and returns the result as a dict. Having a traversal function makes this very easy to code.
...
In contrast to the last function which filled and returned a dict
...
## Summary
- A collection of connected Nodes represents a tree data structure.
- Any connected Node can be accessed from another Node. The root Node is 
the most convenient starting point since all other Nodes are descended from it.
- Tree traversal algorithms can be used to acccess each Node exactly once in 
a specific order.
- Traversals that visit children before parents, or vice-versa, are 
topologically sorted, and can be used to build functions for calculating 
information on a tree. 
- Generator functions return an iterable by using the `#!python yield` 
