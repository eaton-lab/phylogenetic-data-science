...
- this is just a fancier version of `Node._traversal_idx`, but instead of 
yielding Nodes it just sets values to attributes on Nodes and the Tree.
- describe what is .height, .idx,
- remove underscores or describe reason for them, or define newer Node.
